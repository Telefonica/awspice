...
do
	...
	do
		...
		methods=$(cat ../awspice/services/$file | grep "def [a-zA-Z]" | awk '{print $2}' | cut -d '(' -f1 | perl -ne 'print "   awspice.services.'$filename'.'$class'.$_"'| grep -v "worker")
...
do
	...
	do
		...
		# service_title=$(echo $class | sed 's/\Service//g' | awk '{print toupper($0)}')  # Service title UPPERCASE
		methods=$(cat ../awspice/modules/$file | grep "def [a-zA-Z]" | awk '{print $2}' | cut -d '(' -f1 | perl -ne 'print "   awspice.modules.'$filename'.'$class'.$_"'| grep -v "worker")
